...
(function () {
    ...
    SAFARI = Boolean(safariMatch) && !CHROME; //because chrome also inserts safari string into users agent
...
//                $ax.event.raiseSyntheticEvent(elementId, "onMove");
    ...
    var _addResize = function(elementId, eventInfo, resizeInfo, options, moveInfo, rotateInfo) {
        ...
        if($ax.public.fn.IsLayer(axObject.type)) {
            ...
            animations.push({
                ...
                type: queueTypes.resize,
                func: function() {
                    //textarea can be resized manully by the users, but doesn't update div size yet, so doing this for now.
    ...
    //    //textarea can be resized manully by the users, use the textarea child to get the current size
